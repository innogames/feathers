...
package feathers.controls.text
{
	 ...
	 */
	public class StageTextBitmapFontTextEditor extends FeathersControl implements ITextEditor
	{
		/**
		 * @private
		 */
		 ...
		 * Constructor.
		 */
		public function StageTextBitmapFontTextEditor()
		{
		...
		/**
		 * An label that displays a snapshot of the native <code>StageText</code>
		...
		override public function render(support:RenderSupport, parentAlpha:Number):void
		{
			...
			stageTextViewPort.x =  Math.floor(starlingViewPort.x + (HELPER_POINT.x * scaleFactor));
			stageTextViewPort.y =  Math.floor(starlingViewPort.y + (HELPER_POINT.y * scaleFactor));
			...
			if(this.textLabel)
			{
				this.textLabel.x = Math.round(Math.round(HELPER_MATRIX.tx) - HELPER_MATRIX.tx) + 1;
				this.textLabel.y = Math.round(Math.round(HELPER_MATRIX.ty) - HELPER_MATRIX.ty) + 1;
			}
		...
		protected function layout(sizeInvalid:Boolean):void
		{
			...
			if(!this._stageTextHasFocus && (stylesInvalid || dataInvalid || sizeInvalid))
			{
				if(this.textLabel)
				{
		...
		protected function refreshViewPort():void
		{
			...
			this.textLabel.width  = this.actualWidth;
			this.textLabel.height = this.actualHeight;
			this.textLabel.clipRect.setTo(0, 0, this.actualWidth, this.actualHeight);
		}
		...
		protected function addedToStageHandler(event:starling.events.Event):void
		{
			...
			if (this.textLabel && !this.textLabel.parent)
			{
			...
			else if (!this.textLabel)
			{
				...
				this.textLabel.clipRect = new Rectangle(0, 0, 1, 1);
				addChild(this.textLabel);
